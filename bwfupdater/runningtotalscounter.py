...
class RunningTotalCounter:
	...
	def trackWithBestCorrelationForTrack(self, srcTrack, tracksToCheck):
			...
			for i in tracksToCheck:
				currentCorr = np.corrcoef(self.dictOfRunningAnalyses[str(srcTrack)], self.dictOfRunningAnalyses[str(i)])[0,1]
				# print "\n","trackWithBestCorrelationForTrack",currentCorr, srcTrack, i
				if currentCorr > maxCorr:
					maxCorr = currentCorr
					track = i
			return track			

		#this is a first stab at a track analyzer, should they decide to go with remediation instead of flagging bad files
		#i'll want to do correlation analysis with the actual data instead of volume snapshots.
	def performAnalysis(self):
		...
		# we lop off the first two and last three samples as they might be tones or full energy/silence/music
		averagedAnalysis = {}
		for chPos in range(0, self.numberOfTracks):
		...
		if loudestTrack[0] == '2' and quietestTrack[0] == '3':
			...
			#we only capture this for information purposes
			...
			#also this is a non-logarithmic comparison which might be better
			...
			#if the left/right and left/right surr don't correlate than pass unknown
		...
		elif quietestTrack[0] == '3':
			#attempt to establish that it is SMPTE by finding the quitest track
			#if the left/right and left/right surr don't correlate than pass unknown
			...
			# lastly let's check that the for some reason 2 isn't higher in volume than 
			# additionally, we can check to see if  0,1 correlate and 4,5 correlate and 2 is greater than 3 
			doTheFirstTwoTracksCorrelate = self.doesSourceTrackCorrelateWithDestinationTrack(0,1,[1,4,5])
			doTheLastTwoTracksCorrelate = self.doesSourceTrackCorrelateWithDestinationTrack(4,5,[0,1,5])
			
			if doTheFirstTwoTracksCorrelate and doTheLastTwoTracksCorrelate:
				resultDict['LRCorrelation'] 	= True
				resultDict['LSRSCorrelation'] 	= True

				if averagedAnalysis['2'] > averagedAnalysis ['3']:
					resultDict['layout'] = 'SMPTE'

			#fall through is UNKNOWN
		...
		if resultDict['layout'] == 'SMPTE':
			resultDict['status'] = 'pass'

		return resultDict

	def displayStandardDeviation(self, analysisDict):
		for chPos in range(0,self.numberOfTracks):
			std = np.std(analysisDict[str(chPos)][3:-3])
			rms = math.sqrt(std)
	...
	def convertLinearAnalysisToLog(self, analysis):
		outDict = {}
		for key in analysis:
			# outDict[key] = self.decibelConversion(analysis[key]**2)
	...
	def runningAnalysisDecibelRepresentation(self, runningAnalysis):
		...
		# rms = math.sqrt(np.sum(np.power(runningAnalysis, 2))/(self.numberOfSamples*length))
		if rms == 0 or rms == 0.0:
			rms = 0.001
		dbRep = 20 * math.log10(1.0 * rms/(self.maxSampleValue))
		return dbRep	
